// This is a backup copy for the main page
...
fetch(endpoint) // Let's fetch the json from the link given
  .then((blob) => blob.json())
...
function displayMatches() {
    ...
    .map((place) => {
      ...
      return `
            <li>
                ...
                <span class="poulation">${numberWithCommas(
                  place.population
                )}</span>
            </li>
        `;
  ...
  if (this.value !== "") {
    // If the input field has values, let's put some data
    ...
    // Return to original state if the input field is empty
